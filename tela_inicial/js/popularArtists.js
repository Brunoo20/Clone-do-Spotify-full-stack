$(document).ready(function () {
    ...
    function renderPopularArtists(data) {
        ...
        const title = $('<h2>').text('Artistas populares').addClass('section-title popular-artists');
        const grid = $('<div>').addClass('popular-artists-grid');
        const showAllLink = $('<a>').text('Mostrar tudo').addClass('content__link-All-popular-artists').attr('href', '#');
        const title2 = $('<h2>').text('Artistas populares').addClass('section-title popular-artists2').hide();
        ...
        function renderArtists(limit) {
            grid.empty();
            const artists = (data.results.popularArtists?.popularArtists || []).slice(0, limit);
