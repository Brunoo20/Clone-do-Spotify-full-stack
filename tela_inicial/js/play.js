...
function updateGradientReproductionBar() {
    ...
    reproductionBar.css('background', `linear-gradient(to right, ${color} ${percentage}%, #666 ${percentage}%)`)
...
function updatePlaybackPosition() {
    ...
    player.getCurrentState().then(state => {
        ...
        if (justPaused && position === 0 && currentUri) {
            position = isPodcast ? podcastPositions[currentUri] : trackPositions[currentUri]
        }


        // Se houve um seek recente, ignorar a posição do estado até que ela esteja próxima da buscada
        if (lastSeekPosition !== null && Math.abs(position - lastSeekPosition) > 1000) {
        ...
        .done(response => {
            ...
            if (response.success) {
                ...
                if (dataToUse) {
                    ...
                    console.warn(`Nenhum dado retornado para ${contentType}, tentando buscar manualmente`);
            ...
            player.addListener('player_state_changed', (state) => {
                ...
                if (isPodcast) {
                    ...
                    if (currentUri !== lastTrackUri) {
                        ...
                        const newIndex = topTracks.findIndex(track => track.uri === currentUri)
                        if (newIndex !== -1) {
                ...
                if (!isPodcast && state.paused && !state.loading && topTracks.length > 0) {
                    const currentIndex = topTracks.findIndex(track => track.uri === currentTrackUri);
                    ...
                    if (currentIndex !== -1 && state.position >= state.duration - 20000) {
                        if (currentIndex + 1 < topTracks.length) {
                            ...
                            console.log('Tocando próxima música da fila:', nextTrack.name)
                            isPlayingPodcast = true
                            isPlayingBestResult = true
                ...
                if (currentPodcastUri) {
                    const currentCard = $(`.episodes-podcasts-card[data-podcast-uri="${currentPodcastUri}"]`);
                ...
                if (!isSeeking && lastSeekPosition === null) {
                    ...
                    const isPodcastToUse = currentUriToUse?.startsWith('spotify:episode:')

                    if (justPaused && position === 0 && currentUriToUse) {
                        position = isPodcastToUse ? (podcastPositions[currentUriToUse] || 0) : (trackPositions[currentUriToUse] || 0)
                    }
                    ...
                    if (isPlayingPodcast && isPlayingBestResult && !positionUpdateInterval) {
                        positionUpdateInterval = setInterval(updatePlaybackPosition, 1000);
                        justPaused = false;
                    } else if (!isPlayingPodcast && isPlayingBestResult && positionUpdateInterval) {
                        clearInterval(positionUpdateInterval);
                        positionUpdateInterval = null;
                        justPaused = true;
                    }

                    if (position === 0 && currentUriToUse && isPlayingPodcast && isPlayingBestResult) {
                        const savedPosition = isPodcast ? (podcastPositions[currentUriToUse]) : (trackPositions[currentUriToUse])

                        if (savedPosition > 0) {
                            console.log(`Forçando seek para posição salva: ${savedPosition}ms`)
                            seek(savedPosition)
                        }
                    }
                }
...
$(document).ready(function () {
    if (typeof window.initSearch === 'function') {
        ...
        console.error('initSearch não está definido. Verifique se searchHandler.js foi carregado corretamente.')
    }
    ...
    function updateGradientVolumeBar() {
        ...
        volumeBar.css('background', `linear-gradient(to right, ${color} ${volume}%, #666 ${volume}%)`)
    }
    ...
    function togglePlayPause() {
        ...
        if (isPlayingPodcast && isPlayingBestResult) {
            ...
            const isPodcast = currentUri.startsWith('spotify:episode:');
            const positionMs = isPodcast ? (podcastPositions[currentUri] || 0) : (trackPositions[currentUri] || 0);
            ...
            sendSpotifyRequest('play', {
                ...
                .done(function (response) {
                    if (response.success) {
                        console.log('Reprodução retomada com sucesso na posição:', positionMs);
                        isPlayingPodcast = true;
                        isPlayingBestResult = true
    ...
    next.on('click', function (e) {
        ...
        // Verificar contexto de reprodução
        player.getCurrentState().then(state => {
            if (topTracks && topTracks.length > 1 && currentTrackIndex + 1 < topTracks.length) {
                isPlayingBestResult = true
                isPlayingPodcast = true
